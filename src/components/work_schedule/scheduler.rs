...
pub async fn start_scheduler(
    ...
    tokio::spawn(async move {
        loop {
            ...
            let now = Local::now();
            let next_notification_hour = daily_time
                .split(':')
                .next()
                .unwrap_or("8")
                .parse::<u32>()
                .unwrap_or(8);
            let next_notification_minute = daily_time
                .split(':')
                .nth(1)
                .unwrap_or("0")
                .parse::<u32>()
            ...
            let mut next_notification = now
                .date_naive()
                .and_hms_opt(next_notification_hour, next_notification_minute, 0)
            ...
            let next_weekly_notification_hour = weekly_time
                .split(':')
                .next()
                .unwrap_or("8")
                .parse::<u32>()
                .unwrap_or(8);
            let next_weekly_notification_minute = weekly_time
                .split(':')
                .nth(1)
                .unwrap_or("0")
                .parse::<u32>()
            ...
            let mut next_weekly_notification = now
                .date_naive()
            ...
            let next_notification = if next_notification.and_utc().timestamp()
                <= next_weekly_notification.and_utc().timestamp()
            {
            ...
            info!(
                ...
                next_notification.0, next_notification.1
            );
            ...
            if next_notification.0 == "daily" {
                ...
                if let Err(e) =
                    send_daily_notification(&ctx_clone, channel_id, &handle_clone, &today).await
                {
                ...
                if let Err(e) = send_weekly_notification(
                    &ctx_clone,
                    channel_id,
                    &handle_clone,
                    &start_date,

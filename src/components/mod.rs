...
impl ComponentManager {
    ...
    pub async fn init_all(&self, ctx: &serenity::Context, config: Arc<RwLock<Config>>, redis_handle: RedisActorHandle) -> BotResult<()> {
        for component in &self.components {
            info!("Initializing component: {}", component.name());
            
            if let Err(e) = component.init(ctx, config.clone(), redis_handle.clone()).await {
    ...
    pub fn get_component_by_name(&self, name: &str) -> Option<&dyn Component> {
        self.components.iter().find(|c| c.name() == name).map(|c| c.as_ref())
    }
} 
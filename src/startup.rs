...
use crate::components::{ComponentManager, google_calendar::GoogleCalendar};
...
pub async fn start_bot(config: Arc<RwLock<Config>>) -> miette::Result<()> {
    ...
    let (mut redis_actor, redis_handle) = crate::components::redis_service::RedisActor::new(Arc::clone(&config));
    ...
    let client_result = poise::serenity_prelude::ClientBuilder::new(token, intents)
        .framework(poise::Framework::new(options, move |ctx, ready, framework| {
            Box::pin(async move {
                ...
                if let Err(e) = components.init_all(ctx, Arc::clone(&config), redis_handle.clone()).await {

use crate::model::{WorkDay, WorkSchedule, WorkDayExtraction};
...
pub async fn parse_schedule_image(employee_name: &str, image_data: &[u8]) -> Result<WorkSchedule, String> {
    ...
    #[cfg(feature = "web-interface")]
    {
        ...
        let model = env::var("GEMINI_MODEL")
            .unwrap_or_else(|_| "gemini-2.5-pro-exp-03-25".to_string());
        ...
        let agent = gemini_client.agent(&model)
        ...
        for day in extracted_days {
            ...
            if let Ok(_date) = NaiveDate::parse_from_str(&day.date, "%Y-%m-%d") {
                if day.work_hours.is_empty() {
                    ...
                    let is_day_off = day.work_hours.to_lowercase() == "x" || day.work_hours.to_lowercase() == "v";
...
fn extract_json_array(text: &str) -> Result<Vec<WorkDayExtraction>, String> {
    ...
    if let (Some(start), Some(end)) = (clean_text.find('['), clean_text.rfind(']')) {
        if start < end {
            ...
            match from_str(json_part) {
                Ok(days) => return Ok(days),
                Err(e) => {
                    error!("Found cleaned JSON-like structure but couldn't parse it: {}", e);
    ...
    if lines.iter().any(|l| l.contains("\"date\"") && l.contains("\"work_hours\"")) {

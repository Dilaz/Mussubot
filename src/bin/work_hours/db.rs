...
impl WorkHoursDb for RedisDB {
    async fn get_schedule(&self, employee_name: &str) -> Result<Option<WorkSchedule>, String> {
        ...
        // Parse the JSON
        let schedule: WorkSchedule = serde_json::from_str(&data)
    ...
    async fn set_schedule(&self, employee_name: &str, schedule: &WorkSchedule) -> Result<(), String> {
        ...
        for day in &schedule.days {
            ...
            let day_key = format!("{}{}:{}", keys::WORK_HOURS_DAY_PREFIX, employee_name, day.date);
        ...
        info!("Stored schedule for {} with {} days", employee_name, schedule.days.len());
        Ok(())
    }
    ...
    async fn list_employees(&self) -> Result<Vec<String>, String> {
        ...
        let employees: Vec<String> = conn.smembers(keys::WORK_HOURS_EMPLOYEES)
            .await
    ...
    async fn delete_schedule(&self, employee_name: &str) -> Result<(), String> {
        ...
        let dates: Vec<String> = conn.smembers(&dates_key)
            .await
            .map_err(|e| format!("Redis SMEMBERS error: {}", e))?;
